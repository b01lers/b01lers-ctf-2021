...
int main (int argc, char** argv) {
  ...
  if (!p_png) {
    fprintf(stderr,"Can't create png_structp for picture img\n");
    fclose(pfile);
    return 1;
  }
  ...
  for (int y=0; y < p_h; y++) {
    p_rowptrs[y] = malloc(png_get_rowbytes(p_png, p_inf));
  }
  ...
  if (!o_png) {
    fprintf(stderr,"Can't create png_structp for output img\n");
    fclose(ofile);
    return 1;
  }
  ...
  if (!o_inf) {
    fprintf(stderr,"Can't create png_infop for output img\n");
    fclose(ofile);
    return 1;
  }
